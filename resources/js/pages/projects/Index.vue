            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="bg-gradient-to-r from-teal-600 to-teal-500 px-6 py-5">
                    ...
                    <p class="text-teal-100 text-sm mt-1">
                        {{ props.projects.total }} {{ props.projects.total === 1 ? 'project' : 'projects' }} total
                    </p>
                </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="project in props.projects.data" :key="project.id"
                            ...
                            <TableCell class="px-6 py-4">{{ project.name }}</TableCell>
                            <TableCell class="px-6 py-4">{{ project.owner?.name || '-' }}</TableCell>
                            <TableCell class="px-6 py-4">{{ project.team?.name || '-' }}</TableCell>
                            <TableCell class="px-6 py-4 capitalize">{{ project.status }}</TableCell>
                            <TableCell class="px-6 py-4">{{ project.start_date ? new
                                Date(project.start_date).toLocaleDateString() : '-' }}</TableCell>
                            <TableCell class="px-6 py-4">{{ project.due_date ? new
                                Date(project.due_date).toLocaleDateString() : '-' }}</TableCell>
